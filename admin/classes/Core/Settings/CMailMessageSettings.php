...
class Core_Settings_CMailMessageSettings
{
	...
	{
	    //Add a hack in order to be able to modify the emails message.
        $message = addslashes(trim($_REQUEST['mailmessages']));
        $id = (int)$_GET['id'];
		$sql = "UPDATE mail_messages_table SET mail_msg_subject='".trim($_REQUEST['mail_msg_subject'])."',mail_msg = '" .$message ."' WHERE mail_msg_id=" .$id; 
		$query = new Bin_Query();		
		if($query->updateQuery($sql))
        {		
		  $_SESSION['successmsg']= "<div class='alert alert-success'><button data-dismiss='alert' class='close' type='button'>x</button>Updated Successfully</div>";	
        }
        else {
            $_SESSION['successmsg']= "<div class='alert alert-success'><button data-dismiss='alert' class='close' type='button'>x</button>Updated Unsuccessfully</div>";  
        }	
	}
}
?>