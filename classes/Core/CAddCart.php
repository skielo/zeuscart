                ...
                if ($_SESSION['user_id'] != '') {
                    ...
                    if ($cartid != 0)// if cart available for the user
                    {
                        ...
                        if ($query -> executeQuery($sql)) {
                            ...
                            //insert the cart id , product id , product qty(1)
                            if ($soh != 0) {
                                //Make a modification to get all the needed parameters in the shopping cart products
                                $sql_statment= 'SELECT msrp,shipping_cost from products_table where product_id=' . $_GET['prodid'];
                                $query_exe = new Bin_Query();

                                $query_exe -> executeQuery($sql_statment);
                                $originalprice = $query_exe -> records[0]['msrp'];
                                $shipping_cost = $query_exe -> records[0]['shipping_cost'];

                                $sql ="INSERT INTO `shopping_cart_products_table` (`cart_id`,`product_id`,`product_qty`,`date_added`,`product_unit_price`,`shipping_cost`,`variation_id`,`original_price`,`gift_product`)
VALUES ('".$cartid ."','" .$_GET['prodid'] ."',1,'" .date('Y-m-d') ."','" .$originalprice ."','" .$shipping_cost ."',0,'" .$originalprice ."',0)"; 
                                $query = new Bin_Query();
                                $query -> updateQuery($sql);
                            } else {
                    ...
                    else// if cart is not available for the user  cnt ==0
                    {
                        ...
                        $cartid = $this -> getCartIdOfUser();
                        $sql1 = "INSERT INTO `shopping_cart_products_table` (`cart_id`,`product_id`,`product_qty`,`date_added`,`product_unit_price`,`shipping_cost`,`variation_id`,`original_price`,`gift_product`)
VALUES ('".$cartid ."','" .$_GET['prodid'] ."',1,'" .date('Y-m-d') ."','" .$originalprice ."','" .$shipping_cost ."',0,'" .$originalprice ."',0)"; 
                ...
                if ($_SESSION['user_id'] != '') {
                    ...
                    if ($cartid != 0)// if cart available for the user
                    {
                        ...
                        if ($query -> executeQuery($sql)) {
                            ...
                            if ($soh != 0) {
                                $sql =  "INSERT INTO `shopping_cart_products_table` (`cart_id`,`product_id`,`product_qty`,`date_added`,`product_unit_price`,`shipping_cost`,`variation_id`,`original_price`,`gift_product`)
VALUES ('".$cartid ."','" .$_GET['prodid'] ."','" .$qtyfrmproduct ."','" .date('Y-m-d') ."','" .$msrp ."','" .$shipping_cost ."','".(int)$_POST['variations'] ."',0,'".$_GET['vid'] ."')"; 
            ...
            foreach ($_SESSION['mycart'] as $key => $val) {
                ...
                if ($val['qty'] <= $soh) {
                    ...
                    if (in_array($val['product_id'], $cartproducts)) {
                        ...
                        $sql =  "INSERT INTO `shopping_cart_products_table` (`cart_id`,`product_id`,`product_qty`,`date_added`,`product_unit_price`,`shipping_cost`,`variation_id`,`original_price`,`gift_product`)
VALUES ('".$cartid ."','" .$_GET['prodid'] ."','" .$val['qty'] ."','" .$date ."','" .$msrp ."','" .$shippingcost ."', 0, 0, 0)"; 
                    }
                    $query = new Bin_Query();
                ...
                if ($_SESSION['mycart'][0]['cartid'] == '') {
                    ...
                    foreach ($_SESSION['mycart'] as $key => $val) {
                        ...
                        if ($_GET['action'] != 'validatecoupon') {
                            $sqlinsert = "INSERT INTO `shopping_cart_products_table` (`cart_id`,`product_id`,`product_qty`,`date_added`,`product_unit_price`,`shipping_cost`,`variation_id`,`original_price`,`gift_product`)
VALUES ('".$cartid ."','" .$val['product_id'] ."','" .$val['qty'] ."','" .date('Y-m-d') ."','" .$msrp ."','" .$query -> records[0]['shipingamount'] ."', '" .$val['variation_id'] ."','" .$original_price ."','" .$val['gift'] ."')"; 
